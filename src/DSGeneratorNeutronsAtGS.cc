...
void DSGeneratorNeutronsAtGS::GenerateAngularSpectrum() {
  ...
  // DSLog(debugging) << "Costheta: " << costheta << " " << xrand << " " <<
  // sampled_bin <<endlog;
...
void DSGeneratorNeutronsAtGS::ShootEnergyFission() {
  ...
  if (!samplerInitialized) {
    ...
    DSLog(trace) << "Total neutron rate above 1 MeV "
                 << "12 neutrons/m2 h (wet rock)" << endlog;
    DSLog(trace) << "Total neutron rate above 1 MeV "
                 << "21 neutrons/m2 h (dry rock)" << endlog;
  ...
  //  for (j=0;j<nbin;j++)
  //     {
  //       G4cout << j << " " << energybin[j]/MeV << " " << userpdf[j] <<
  //       G4endl;
  //     }
  ...
  if (userpdf[j + 1] != userpdf[j]) {
    ...
    //" " << energybin[j] << " " << energybin[j+1] << " " << particle_energy <<
    //endlog;
  } else
...
void DSGeneratorNeutronsAtGS::ShootEnergyMuons() {
  ...
  // DSLog(debugging) << "Particle energy " << particle_energy/MeV << " MeV" <<
  // endlog;
  return;
}

void DSGeneratorNeutronsAtGS::SetParticleDefinition(G4ParticleDefinition* aParticleDefinition) {
...
void DSGeneratorNeutronsAtGS::DSGeneratePrimaries(G4Event* evt) {
  ...
  DSLog(debugging) << "       Energy: " << particle_energy / keV << " keV" << endlog;
  DSLog(debugging) << "     Position: " << particle_position / cm << " cm; (" << neutronFrom << ")" << endlog;
  DSLog(debugging) << "    Direction: " << particle_momentum_direction << endlog;
  DSLog(debugging) << " NumberOfParticlesToBeGenerated: " << NumberOfParticlesToBeGenerated << endlog;
  ...
  //   file2 << particle_momentum_direction.x() << " " <<
  //   particle_momentum_direction.y() <<
