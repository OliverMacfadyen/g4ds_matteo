...
DSDetectorPMTDS50::DSDetectorPMTDS50(G4VPhysicalVolume* myMotherVolume, G4VPhysicalVolume* myTeflonCapDiskVolume) {
  ...
  if (fMotherVolume->GetRotation() == 0) {
    ...
    DSLog(routine) << " Constructing Bottom TPC PMTs Geometry  " << endlog;
  ...
  */
  // PMT Surrounding
  fSolidPMTSurrounding = new G4Tubs("PMTSurrounding_Solid", 0, myPMTHead_d / 2., myTeflonCap_h / 2., 0, myTwoPi);
  // Part of the PMT inside the surrounding (Head, vacuum, window and body)
  fSolidPMTHead = new G4Tubs("PMTHead_Solid", 0, myPMTHead_d / 2., myPMTHead_h / 2., 0, myTwoPi);

  fSolidPMTHeadVac = new G4Tubs("PMTHeadVac_Solid", 0, myPMTHead_d / 2. - myPMTWall, myPMTHead_h / 2. - myPMTWall / 2., 0, myTwoPi);

  fSolidPMTWindow = new G4Tubs("PMTWindow_Solid", 0, myPMTHead_d / 2. - myPMTWall, myPMTWindowThickness / 2., 0, myTwoPi);

  fSolidPMTJoinVac = new G4Tubs("PMTJoinVac_Solid", 0, myPMTBody_d / 2. - myPMTWall, myPMTWall / 2., 0, myTwoPi);

  fSolidPMTBody = new G4Tubs("PMTBody_Solid", 0, myPMTBody_d / 2., myTeflonCap_h / 2. - myPMTHead_h / 2. - myPMTOffset / 2., 0, myTwoPi);

  fSolidPMTBodyVac = new G4Tubs("PMTBodyVac_Solid", 0, myPMTBody_d / 2. - myPMTWall, myTeflonCap_h / 2. - myPMTHead_h / 2. - myPMTOffset / 2., 0, myTwoPi);

  // Part of the PMT body outside the surrounding
  fSolidPMTTop = new G4Tubs("PMTTop_Solid", 0, myPMTBody_d / 2., myPMTBody_h / 2. - myTeflonCap_h / 2. + myPMTOffset / 2., 0, myTwoPi);

  fSolidPMTTopVac = new G4Tubs("PMTTopVac_Solid", 0, myPMTBody_d / 2. - myPMTWall, myPMTBody_h / 2. - myTeflonCap_h / 2. + myPMTOffset / 2. - myPMTWall / 2., 0, myTwoPi);

  fSolidPMTLArDisk = new G4Tubs("PMTLArDisk_Solid", 0, myPMTHead_d / 2., myPMTOffset / 2., 0, myTwoPi);

  fSolidPMTStem = new G4Tubs("PMTStem_Solid", 0, myPMTBody_d / 2. - myPMTWall, myPMTWall / 2., 0, myTwoPi);
  ...
  G4ThreeVector myPMTStemPos(0, 0, fSolidPMTTop->GetZHalfLength() - myPMTWall / 2.);
  ...
  */
  ...
  for (int i = 0; i < 19; i++) {
    ...
    fLogicPMTWindow[i] = new G4LogicalVolume(fSolidPMTWindow, DSMaterial::Get()->GetFakePhotocathode(), myPMTWindowLogicName[i]);
    ...
    fLogicPMTStem[i] = new G4LogicalVolume(fSolidPMTStem, DSMaterial::Get()->GetBoroSiliGlass(), myPMTStemLogicName[i]);
...
void DSDetectorPMTDS50::DefineSurfaces() {
  ...
  fPMTVacuumSurfProp->AddConstProperty("REFLECTIVITY", 1.0, true);
  fPMTVacuumSurfProp->AddConstProperty("EFFICIENCY", 0.0, true);
  ...
  // for(int i = 0; i < 19; i++)  fPMTLArSurface[i] = new
  // G4LogicalBorderSurface("PMTLArSurface", fPhysicPMTWindow[i],
  // fPhysicPMTLArDisk[i],  fOpPMTLArSurface );
  ...
  for (int i = 0; i < 19; i++) fTeflonLArSurface[i] = new G4LogicalBorderSurface("TeflonLArSurface", fPhysicPMTLArDisk[i], fTeflonCapDiskVolume, fOpTeflonLArSurface);
  fOpTeflonLArSurface->SetType(dielectric_metal);
  fOpTeflonLArSurface->SetModel(unified);
  fOpTeflonLArSurface->SetSigmaAlpha(0.1);
  // where sigma_alpha is in [rad]
  fOpTeflonLArSurface->SetFinish(groundfrontpainted);
  ...
  fOpTeflonLArSurface->SetMaterialPropertiesTable(fLArTeflonSurfProp);

  // PMT Wall - LAr
  G4OpticalSurface* fOpWallLArSurface = new G4OpticalSurface("OpWallLArSurface");
  for (int i = 0; i < 19; i++) new G4LogicalBorderSurface("WallLArSurface", fPhysicPMTLArDisk[i], fPhysicPMTHead[i], fOpWallLArSurface);
  fOpWallLArSurface->SetType(dielectric_metal);
  fOpWallLArSurface->SetModel(unified);
  fOpWallLArSurface->SetFinish(groundfrontpainted);
  fOpWallLArSurface->SetSigmaAlpha(0.1);

  G4MaterialPropertiesTable* fWallLArSurfProp = new G4MaterialPropertiesTable();
  G4double WallLArENE[4] = {0.1 * eV, 8.0 * eV, 8.3 * eV, 20.0 * eV};
  G4double REFUVP = 0.1;
  G4double REFVISP = 0.5;
  G4double WallLArREF[4] = {REFVISP, REFVISP, REFUVP, REFUVP};
  fWallLArSurfProp->AddProperty("REFLECTIVITY", WallLArENE, WallLArREF, 4);
  fOpWallLArSurface->SetMaterialPropertiesTable(fWallLArSurfProp);
}

/*
 * $Log: DSDetectorPMTDS50.cc,v $
 * Revision 1.4  2016/03/03 23:13:28  cz2
 * Added two boundaries LArdisk->Teflon and LArdisk->PMTWall.
 *
 * Revision 1.3  2015/09/03 10:01:56  pagnes
