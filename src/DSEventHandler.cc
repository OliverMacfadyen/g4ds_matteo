...
void DSEventHandler::SetClusterPosition(G4ThreeVector val) {
  for (int i = 0; i < 3; i++) theClusterStructure.Position[i] = val[i];
}
...
void DSEventHandler::DumpEvent() {
  ...
  DSLog(trace) << "Event: NCluster  " << theEventStructure.NClusters << endlog;  
  DSLog(trace) << " " << endlog;
}
...
void DSEventHandler::DumpPhoton() {
  for (int i = 0; i < int(theEventStructure.thePhotons.size()); i++) {
    ...
    for (int j = 0; j < 3; j++) DSLog(trace) << "Photon: Position      " << j << " " << theEventStructure.thePhotons[i].Position[j] << endlog;
    DSLog(trace) << endlog;
  }
}
...
void DSEventHandler::DumpCluster() {
  for (int i = 0; i < int(theEventStructure.theClusters.size()); i++) {
    DSLog(trace) << "clus: fp   " << theEventStructure.theClusters[i].RecoilID << endlog;
    DSLog(trace) << "clus: E    " << theEventStructure.theClusters[i].Energy << endlog;
    DSLog(trace) << "clus: S1   " << theEventStructure.theClusters[i].S1Energy << endlog;
    DSLog(trace) << "clus: S2   " << theEventStructure.theClusters[i].S2Energy << endlog;
    //DSLog(trace) << "clus: Mat  " << theEventStructure.theClusters[i].Material << endlog;
    DSLog(trace) << "clus: T    " << theEventStructure.theClusters[i].Time << endlog;
    DSLog(trace) << "clus: X    " << theEventStructure.theClusters[i].Position[0] << endlog;
    DSLog(trace) << "clus: Y    " << theEventStructure.theClusters[i].Position[1] << endlog;
    DSLog(trace) << "clus: Z    " << theEventStructure.theClusters[i].Position[2] << endlog;
    DSLog(trace) << endlog;
  }
}
...
void DSEventHandler::ClearEvent() {
  ...
  (theEventStructure.theClusters).clear();
}
...
void DSEventHandler::ClearCluster() {
  theClusterStructure.RecoilID = 0;
  //theClusterStructure.Material = 0;
  theClusterStructure.Energy = 0;
  theClusterStructure.S1Energy = 0;
  theClusterStructure.S2Energy = 0;
  theClusterStructure.Time = 0;
  theClusterStructure.Position[0] = 0;
  theClusterStructure.Position[1] = 0;
  theClusterStructure.Position[2] = 0;
}
