...
DSDetectorLicorne::DSDetectorLicorne(G4VPhysicalVolume *myMotherVolume) {
  fMotherVolume = myMotherVolume;
  DSLog(routine) << " Constructing Licorne Setup Geometry" << endlog;
  ...
  // ouble myTeflonOuterRadius = myTPCOuterRadius + myTeflonThickness;
  ...
  // G4double cryoOuterDomeThickness = 0.88 * 2.54 * cm;
  // G4double cryoOuterDomeRadius = 8.00 * 2.54 / 2 * cm;
  ...
  // G4double TPCLArOuterH = GetCm(3.00/2)*cm;
  ...
  // G4double PMTBottomBody1H =  (SleeveOuterH - MainBodyH + LArTPCShift.getZ())/2 ;
  ...
  // G4double PMTTopPhotoCathodeL = 2.05/2 * cm;
  ...
  // G4double PMTTopBody2H_Vac = PMTTopBody2H - 0.2 * cm;
  ...
  if (DSStorage::Get()->GetLicExpHall()) {
    G4Box *fSolidFloor = new G4Box("Floor_Solid", 10 * m, 10 * m, 0.5 * m);
    G4LogicalVolume *fLogicLicorneFloor = new G4LogicalVolume(fSolidFloor,
                                                              DSMaterial::Get()->GetConcrete(),
                                                              "LicorneFloor_Logic");
    G4ThreeVector floorShift(0, 0, -1.70 * m);

    fPhysicLicorneFloor = new G4PVPlacement(0, floorShift, "LicorneFloor", fLogicLicorneFloor, fMotherVolume, false, 0, myCheckOverlap);

    /*G4Box           *fSolidWall = new G4Box("Wall_Solid",0.25*m,5*m,1.50*m);
    G4LogicalVolume   *fLogicLicorneWall = new G4LogicalVolume(fSolidWall,
                 DSMaterial::Get()->GetConcrete(),
               "LicorneRock_Logic");
    G4ThreeVector wallShift (5.5*m,0*m,0*m);
    G4PVPlacement     *fPhysicLicorneWall = new G4PVPlacement(0,
                    wallShift,
              "LicorneWall",
               fLogicLicorneWall,
               fMotherVolume,
    ...
    G4Box           *fSolidWall1 = new G4Box("Wall_Solid1",5.5*m,0.25*m,1.50*m);
    G4LogicalVolume   *fLogicLicorneWall1 = new G4LogicalVolume(fSolidWall1,
                  DSMaterial::Get()->GetConcrete(),
                  "LicorneRock_Logic");

    G4ThreeVector wallShift1 (0,-1.5*m,0);
    G4PVPlacement     *fPhysicLicorneWall1 = new G4PVPlacement(0,
                   wallShift1,
             "LicorneWall1",
             fLogicLicorneWall1,
             fMotherVolume,
             ...
             0,
             myCheckOverlap);
     G4ThreeVector wallShift2 (0,1.5*m,0);

     G4PVPlacement     *fPhysicLicorneWall2 = new G4PVPlacement(0,
                    wallShift2,
              "LicorneWall2",
              fLogicLicorneWall1,
              fMotherVolume,
  ...
  /*
   ...
   G4Tubs	*fSolidInactiveGar = new G4Tubs("InactiveGar_Solid",
              SleeveOuterR,
            cryoInnerR - cryoInnerThickness ,
            (cryoInnerH-cryoInnerThickness - TPCLArOuterH + TPCGArOuterH + 0.33*cm)/2,
            0,
            2*M_PI);

  G4LogicalVolume *fLogicInactiveGar = new G4LogicalVolume(fSolidInactiveGar,
                   DSMaterial::Get()->GetGaseousArgon(),
                 "SolidInactiveGar_Logic");

  G4ThreeVector myInactiveGarShift (0,0,(cryoInnerH-cryoInnerThickness + TPCLArOuterH - TPCGArOuterH - 0.33*cm)/2);

  G4PVPlacement   *fPhysicInactiveGar = new G4PVPlacement(0,
                  myInactiveGarShift,
                "InactiveGar",
                fLogicInactiveGar,
                fPhysicInactiveLar,
   ...
   G4Tubs	*fSolidInactiveGarTop = new G4Tubs("InactiveGarTop_Solid",
              0,
            SleeveOuterR ,
            (cryoInnerH-cryoInnerThickness-SleeveOuterH)/2,
            0,
            2*M_PI);


  G4LogicalVolume *fLogicInactiveGarTop = new G4LogicalVolume(fSolidInactiveGarTop,
                   DSMaterial::Get()->GetGaseousArgon(),
                 "SolidInactiveGarTop_Logic");

  G4ThreeVector myInactiveGarShiftTop (0,0,(cryoInnerH-cryoInnerThickness+ SleeveOuterH)/2 );

  G4PVPlacement   *fPhysicInactiveGarTop = new G4PVPlacement(0,
                  myInactiveGarShiftTop,
                "InactiveGarTop",
                fLogicInactiveGarTop,
                fPhysicInactiveLar,
                false,
                0,
                myCheckOverlap );
...
*/
  ...
  /* G4double PMTTopBody1H = (SleeveInnerH - MainBodyH)/2;
   G4double PMTTopBody2H =  28.25 * cm - PMTTopBody1H ;
   G4double PMTTopBody1H = (SleeveInnerH - MainBodyH)/2;
   //G4double PMTTopPhotoCathodeL = 2.05/2 * cm;
   G4double PMTTopBodyL = 2.05/2 * cm;
   G4double PMTTopBodyL_Vac = PMTPhotoCathodeL - 0.2*cm;
   G4double PMTTopPhotoCathodeH  = 0.1 * cm;
 */
  ...
  if (DSStorage::Get()->GetLicWallActivation()) {
    G4double leadBrick_length = 15 * cm;
    G4double leadBrick_high = 5 * cm;
    G4double leadBrick_thick = DSStorage::Get()->GetLicWallThick();

    G4double wallTPCdistance = DSStorage::Get()->GetLicWallDistanceToTPC();

    G4double wallWindowLength = DSStorage::Get()->GetLicWallWindowLength();

    G4Box *boxBrickLeadWall = new G4Box("boxBrickLeadWall", 0.5 * leadBrick_thick, 0.5 * leadBrick_length, 0.5 * leadBrick_high);

    G4LogicalVolume *logicLeadWall = new G4LogicalVolume(boxBrickLeadWall,
                                                         DSMaterial::Get()->GetMetalLead(), "fMotherVolume");

    fPhysicLeadWall[0] = new G4PVPlacement(0,
                                           G4ThreeVector(-cryoOuterR - wallTPCdistance - 0.5 * leadBrick_thick, 0.5 * leadBrick_length, leadBrick_high),
                                           "LeadWall",
                                           logicLeadWall,
                                           fMotherVolume,
                                           false,
    ...
    fPhysicLeadWall[1] = new G4PVPlacement(0,
                                           G4ThreeVector(-cryoOuterR - wallTPCdistance - 0.5 * leadBrick_thick, -0.5 * leadBrick_length, leadBrick_high),
                                           "LeadWall",
                                           logicLeadWall,
                                           fMotherVolume,
    ...
    fPhysicLeadWall[2] = new G4PVPlacement(0,
                                           G4ThreeVector(-cryoOuterR - wallTPCdistance - 0.5 * leadBrick_thick, 0.5 * leadBrick_length, -leadBrick_high),
                                           "LeadWall",
                                           logicLeadWall,
                                           fMotherVolume,
    ...
    fPhysicLeadWall[3] = new G4PVPlacement(0,
                                           G4ThreeVector(-cryoOuterR - wallTPCdistance - 0.5 * leadBrick_thick, -0.5 * leadBrick_length, -leadBrick_high),
                                           "LeadWall",
                                           logicLeadWall,
                                           fMotherVolume,
    ...
    fPhysicLeadWall[4] = new G4PVPlacement(0,
                                           G4ThreeVector(-cryoOuterR - wallTPCdistance - 0.5 * leadBrick_thick, 0.5 * leadBrick_length + 0.5 * wallWindowLength, 0),
                                           "LeadWall",
                                           logicLeadWall,
                                           fMotherVolume,
                                           false,
    ...
    fPhysicLeadWall[5] = new G4PVPlacement(0,
                                           G4ThreeVector(-cryoOuterR - wallTPCdistance - 0.5 * leadBrick_thick, -0.5 * leadBrick_length - 0.5 * wallWindowLength, 0),
                                           "LeadWall",
                                           logicLeadWall,
                                           fMotherVolume,
  ...
  //--------------------------------------------------
  G4VisAttributes* visTransparent = new G4VisAttributes(G4Colour(0, 0, 0, 0));
  G4VisAttributes* visPMT = new G4VisAttributes(G4Colour(0.75, 0.75, 0.75, 1));
  G4VisAttributes* visCopper = new G4VisAttributes(G4Color(0.722, 0.451, 0.2, 0.3));
  G4VisAttributes* visSteel = new G4VisAttributes(G4Color(0.533, 0.545, 0.553, 0.3));
  // G4VisAttributes* visLAr = new G4VisAttributes(G4Color(0.952, 0.952, 0.956, 0.5));  // G4Color(0.831, 0.945, 0.976, 0.5)
  G4VisAttributes* visLAr = visTransparent;
  G4VisAttributes* visLArInactive = visLAr;
  G4VisAttributes* visGAr = visLAr;
  G4VisAttributes* visTeflon = new G4VisAttributes(G4Color(0.25, 0.25, 0.25, 0.3));
  G4VisAttributes* visWindow = new G4VisAttributes(G4Color(0.58, 0.576, 0.69, 0.3));

  fMotherVolume->GetLogicalVolume()->SetVisAttributes(G4VisAttributes::GetInvisible());
  fLogicLicorne->SetVisAttributes(visSteel);
  fLogicLicorneVac->SetVisAttributes(visTransparent);
  fLogicInnerCryo->SetVisAttributes(visSteel);
  fLogicInactiveLar->SetVisAttributes(visLArInactive);
  fLogicExternalSleeve->SetVisAttributes(visTeflon);
  fLogicCopperRing->SetVisAttributes(visCopper);
  fLogicActiveLArTPC->SetVisAttributes(visLAr);
  fLogicGasPocket->SetVisAttributes(visGAr);
  fLogicGrid->SetVisAttributes(visSteel);
  fLogicCathodeWindow->SetVisAttributes(visWindow);
  fLogicAnodeWindow->SetVisAttributes(visWindow);
  for (int i = 0; i < 7; i++) {
    fLogicTopPMTBody1[i]->SetVisAttributes(visPMT);
    fLogicTopPMTBody1_Window[i]->SetVisAttributes(visPMT);
    fLogicTopPMTBody1_Vac[i]->SetVisAttributes(visPMT);
  }
  fLogicBottomPMTWindow->SetVisAttributes(visPMT);
  fLogicBottomPMTBody1->SetVisAttributes(visPMT);
  fLogicBottomPMTBody1Vac->SetVisAttributes(visPMT);
  fLogicBottomPMTBody2->SetVisAttributes(visPMT);
  fLogicBottomPMTBody2Vac->SetVisAttributes(visPMT);
}

DSDetectorLicorne::~DSDetectorLicorne() {
  ;  // delete fMessenger;
}

void DSDetectorLicorne::DefineSurfaces() {
  ////////////////////////////////////////
...
double DSDetectorLicorne::GetCm(double inches) { return inches * 2.54; }
