...
DSDetectorDS10::DSDetectorDS10(G4VPhysicalVolume* myMotherVolume) {
  ...
  // Trunk
  // Note: treat the trunk as a unique volume instead of dividing it into
  // sub-volumes
  ...
  // Inner Vessel: Cylindrical Acrylic Walls + Two Fused Silica Windows Disks
  // (Top and Bottom)
  ...
  //  Grid is simulated as a dielectric. Hence, instead using steel, we use
  //  FusedSilica.

  fSolidGrid = new G4Tubs("Grid_Solid", 0, myLAr_d / 2., myGrid_h / 2., 0, myTwoPi);
  // fLogicGrid  = new G4LogicalVolume( fSolidGrid,
  // DSMaterial::Get()->GetFusedSilica(), "Grid_Logic" );
  ...
  // Active Volume: 3M foils + TPB (simulated as surface properties) + Support
  // Rings
  ...
  G4double myCopperRing_t = 0.0068 * mm;  //  2. * 305*g/m2/DSMaterial::Get()->GetMetalCopper()->GetDensity();
                                          //  //Ref. F
  ...
  // Compression Plate (StainlessSteel Plates at the top and bottom of the TPC
  // holding it together) the space among the PMTs is filled with 1.3 cm thick
  // PTFE reflectors and the exposed stainlesssteel areas are covered with 3M
  // foils
  ...
  // PMTs
  // they are approximated with a cylinder of diameter equal to the photocathode
  // window diameter and formed by three volumes: body (Kovar), photocathode
  // window (FusedSilica), vacuum interior
...
void DSDetectorDS10::DefineSurfaces() {
  ...
  for (int i = 0; i < 14; i++) new G4LogicalBorderSurface("PMTLArSurface", fPhysicLArBath, fPhysicPMTWindow[i], fOpPMTLArSurface);
