...
DSDetectorTester::DSDetectorTester(G4VPhysicalVolume* myMotherVolume) {
  ...
  fPhysicSteelTank = new G4PVPlacement(0, G4ThreeVector(0, 0, 0), "BlackHole", fLogicSteelTank, fMotherVolume, false, 1, DSStorage::Get()->GetCheckOverlap());
  ...
  fPhysicLAr = new G4PVPlacement(0, G4ThreeVector(0, 0, 0), "LAr", fLogicLAr, fPhysicSteelTank, false, 2, DSStorage::Get()->GetCheckOverlap());
  ...
  fPhysicTeflon = new G4PVPlacement(0, G4ThreeVector(0, 0, myLArHeight - myTeflonHeight), "Teflon", fLogicTeflon, fPhysicLAr, false, 3, DSStorage::Get()->GetCheckOverlap());
  ...
  fPhysicTPB = new G4PVPlacement(0, G4ThreeVector(0, 0, myLArHeight - 2 * myTeflonHeight - myTPBHeight), "TPB", fLogicTPB, fPhysicLAr, false, 4, DSStorage::Get()->GetCheckOverlap());
...
void DSDetectorTester::DefineSurfaces() {
  ...
  // G4LogicalBorderSurface("OpLArTeflonBorderSurface", fPhysicTeflon,
  // fPhysicLAr, fOpLArTeflonSurface ); fOpLArTeflonSurface->SetType(
 ...
 * Added a detector tester, with simpplified geometry (configuration number = 4)
 * to test optical properties of the materials
 *
 *
 */
