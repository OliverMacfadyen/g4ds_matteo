...
DSStackingRDMChainMessenger::DSStackingRDMChainMessenger(DSStackingRDMChain* stack) {
  ...
  fStopAtIsotopeCmd = new G4UIcmdWithAnInteger("/ds/stack/rdmchain/stop_at_isotope", this);
  fStopAtIsotopeCmd->SetGuidance("Stop when reaching the chosen isotope");
...
void DSStackingRDMChainMessenger::SetNewValue(G4UIcommand* cmd, G4String newValue) {
  ...
  } else if  (cmd == fStopAtIsotopeCmd) {
    stacking->SetStopUntilIsotope(true);
    stacking->SetStopIsotope(fStopAtIsotopeCmd->ConvertToInt(newValue));
    DSLog(routine) << "Stop decay chain at istope: " << newValue << endlog;
  }
}
