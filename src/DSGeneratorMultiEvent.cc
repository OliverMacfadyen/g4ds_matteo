...
// DSGeneratorMultiEvent::DSGeneratorMultiEvent(const DSGeneratorMultiEvent &
// other)
//{;}
...
void DSGeneratorMultiEvent::DSGeneratePrimaries(G4Event* event) {
  if (!fRead) {
    ...
    for (int i = 0; i < (G4int)fListOfStringParticles.size(); i++) fListOfParticles.push_back(ConvertFromString(fListOfStringParticles[i]));
    ...
    for (G4int i = 0; i < (G4int)fListOfParticles.size(); i++) {
      ...
      DSLog(routine) << i << " " << fParticleTable->FindParticle(pdg)->GetParticleName() << " " << fListOfParticles[i].Energy << " MeV" << endlog;
      if (fListOfParticles[i].Id > Id) {
        fNVertex++;
        Id = fListOfParticles[i].Id;
  ...
  for (G4int i = 0; i < G4int(fListOfParticles.size()); i++) {
    ...
    DSEventHandler::Get()->SetEnergy((DSEventHandler::Get()->GetEnergy() + fListOfParticles[i].Energy * MeV) / keV);
    ...
    DSEventHandler::Get()->SetDEnergy((fListOfParticles[i].Energy * MeV) / keV);
...
MultiEvent DSGeneratorMultiEvent::ConvertFromString(string text) {
  ...
  sscanf(text.c_str(), "%d %f %d %f %f", &myMultiParticle.Id, &myMultiParticle.BRTOT, &myMultiParticle.PDG, &myMultiParticle.Energy, &myMultiParticle.BR);
