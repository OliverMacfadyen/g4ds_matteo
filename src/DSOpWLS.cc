...
//
//
// $Id: DSOpWLS.cc,v 1.2 2016/02/26 10:52:30 pagnes Exp $
// GEANT4 tag $Name:  $
//
////////////////////////////////////////////////////////////////////////
...
DSOpWLS::DSOpWLS(const G4String& processName, G4ProcessType type) : G4VDiscreteProcess(processName, type) {
...
// -------------
//
G4VParticleChange* DSOpWLS::PostStepDoIt(const G4Track& aTrack, const G4Step& aStep) {
  ...
  const G4MaterialPropertyVector* WLS_Intensity = aMaterialPropertiesTable->GetProperty("WLSCOMPONENT");
  ...
  if (aMaterialPropertiesTable->ConstPropertyExists("WLSMEANNUMBERPHOTONS")) {
    G4double MeanNumberOfPhotons = aMaterialPropertiesTable->GetConstProperty("WLSMEANNUMBERPHOTONS");

    ///////////////////////////////////////////////////////////
    // DF modification
    // the WLS does not increase the photon. Either it absorbs or re-emit.
    // Hence, WLSMEANNUMBERPHOTONS becomes the efficiency in re-emitting the
    // photon next line is part of the original code
  ...
  for (G4int i = 0; i < NumPhotons; i++) {
    ...
    G4DynamicParticle* aWLSPhoton = new G4DynamicParticle(G4OpticalPhoton::OpticalPhoton(), photonMomentum);
    aWLSPhoton->SetPolarization(photonPolarization.x(), photonPolarization.y(), photonPolarization.z());
  ...
  if (verboseLevel > 0) { G4cout << "\n Exiting from DSOpWLS::DoIt -- NumberOfSecondaries = " << aParticleChange.GetNumberOfSecondaries() << G4endl; }
  return G4VDiscreteProcess::PostStepDoIt(aTrack, aStep);
}
...
// ---------------
//
G4double DSOpWLS::GetMeanFreePath(const G4Track& aTrack, G4double, G4ForceCondition*) {
  ...
  if (aMaterialPropertyTable) {
    AttenuationLengthVector = aMaterialPropertyTable->GetProperty("WLSABSLENGTH");
    if (AttenuationLengthVector) {
      AttenuationLength = AttenuationLengthVector->Value(thePhotonEnergy);
    } else {
      //      G4cout << "No WLS absorption length vector specified for " <<
...
void DSOpWLS::UseTimeProfile(const G4String name) {
  if (name == "delta") {
    ...
    G4Exception("DSOpWLS::UseTimeProfile", "em0202", FatalException, "generator does not exist");
  }
}
/*
 * $Log: DSOpWLS.cc,v $
 * Revision 1.2  2016/02/26 10:52:30  pagnes
 ...
 * WLSMEANNUMBERPHOTONS defines now the efficiency in re-emitting the photon. 1
 * - WLSMEANNUMBERPHOTONS is the absorbtion probability
 *
 *
 */
