...
G4VParticleChange* G4OpBoundaryProcess::PostStepDoIt(const G4Track& aTrack,
                                                     const G4Step& aStep)
{
  //2022 PA porting our models from G4DS10
  G4bool IsGrid = false;
  G4bool IsGridExit = false;
  G4bool IsArTPB = false;
  // G4bool IsClevios = false;
  ...
  if(type == dielectric_dielectric)
  {
    if(fFinish == polished || fFinish == ground)
    {
      ...
      ///////////////////////////////////////////////////////////////////////
      //2022 - PA import our optical models from G4DS10
      ///////////////////////////////////////////////////////////////////////
      // if ( fMaterial2->GetName() == "GridSteel" )
      // fMaterial1->GetMaterialPropertiesTable()->DumpTable() ;
      // fMaterial2->GetMaterialPropertiesTable()->DumpTable() ;

      if (fOpticalSurface != nullptr ) {   // need to add this block to
        G4MaterialPropertiesTable* aMaterialPropertiesTable = fOpticalSurface->GetMaterialPropertiesTable();
        ...
        if (aMaterialPropertiesTable->ConstPropertyExists("DOITO")) {
          ...
          G4double* myITO = DSStorage::Get()->GetITOParameters(fMaterial1->GetMaterialPropertiesTable()->GetProperty("RINDEX")->Value(fPhotonMomentum), fMaterial2->GetMaterialPropertiesTable()->GetProperty("RINDEX")->Value(fPhotonMomentum), 15.0, hc / (fPhotonMomentum / eV), GetIncidentAngle());
          ...
          if (myITO[0] > 0) {
            ...
            //PA I think what was below is now changed and we both have reflectivity and transmittance with sensible meaning....
            fTransmittance = myITO[1] / (myITO[0] + myITO[1]);
            fReflectivity = myITO[0] / (myITO[0] + myITO[1]);
            // fEfficiency = 0;
            //
            // // theTransmittance =  myITO[1];
            // fReflectivity = myITO[0] + myITO[1];

            fTransmittance =  myITO[1];
            fReflectivity = myITO[0];
          }
        }
        /// Modfied by PA for adding new grid model (transmittance as function of
        /// incident angle) Updated by P. Meyers 5/21/2014

        if (aMaterialPropertiesTable->ConstPropertyExists("DOGRID")) {

          // PDM 5/21/14: An update to the "New grid model."  The transparency is
          // calculated in two parts:
          //  1) The transparency at normal incidence -- the hole/total area,
          //  2) An angle-dependent transparency of a hole due to the thickness of
          //  the grid.
          // Note that we violate usual G4 usage, where fReflectivity is really
          // 1-absorption. For the grid only, fReflectivity is the actual
          // TRANSMISSION.  Sorry!

          G4double GridNormalTransp = DSParameters::Get()->GetGridNormalTransparency();

          // PDM 5/20/14: GetIncidentAngle uses theFacetNormal -- not yet defined.
          // For grid (with current model) it will be specular.
          // pdm	std::cout<<"***G4OBP:theGlobalNormal
          // ="<<theGlobalNormal<<std::endl;
          // 2022 PA changed this

          fFacetNormal = fGlobalNormal;

          G4double myIncAngle = GetIncidentAngle();

          fTransmittance = DSStorage::Get()->GetGridParameters(myIncAngle);
          fTransmittance = fTransmittance * GridNormalTransp;
          fReflectivity = 1 - fTransmittance;
        ...
        if (aMaterialPropertiesTable->ConstPropertyExists("DOLArLayerTPB")) {
          fReflectivity  = 0;
          // if (fTransmittance == 0) { //Why of this?...
            if (DSParameters::Get()->GetWithDefect()) {
              fTransmittance = DSStorage::Get()->GetTPBEfficiency(theGlobalPoint.x(), theGlobalPoint.y());
            } else {
              fTransmittance = 1;
            }
          // }
        ...
        if (aMaterialPropertiesTable->ConstPropertyExists("DOCLEVIOS")) {
          // IsClevios = true;
          fTransmittance = 1;
          fReflectivity  = 0;
        }
    ...
    // PA - START THE SECOND SET OF CHECKS
    ...
    else
    {
      ...
      // 2022 PA port our models from G4SD10
      if (IsGridExit) {
       return G4VDiscreteProcess::PostStepDoIt(aTrack, aStep);
      }

      G4double rand = G4UniformRand();
      if(rand > fReflectivity + fTransmittance)  // ABSORPTION
      {
        DoAbsorption();
      }
      else if(rand > fReflectivity)              // TRANSMISSION
      {
        fStatus          = Transmission;
        fNewMomentum     = fOldMomentum;
        fNewPolarization = fOldPolarization;

        if (IsGrid )  {   // PA trasnmittance trough the GRID
        	    //pdm	    std::cout << " GRID TRANSMISSION  " << std::endl;
        	    return G4VDiscreteProcess::PostStepDoIt(aTrack, aStep);
        }

        //2022 PA port our models from G4SD10
        //this branch does absorption and reflection. PDM 5/22/2014
        if (IsArTPB) {
          // Model "Lambertian transmission".  Do Lambertian
                    // _reflection_, then reverse momentum and polarization.
            fStatus = LambertianReflection;
            DoReflection();
            fStatus = LambertianTransmission;
            fNewMomentum = -fNewMomentum;
            fNewPolarization = -fNewPolarization;
        }
      ...
      else         // REFLECTION
      {
        ...
        //this branch does absorption and reflection. PDM 5/22/2014
        if (IsGrid) {
          double Actual_Grid_Reflectance = DSParameters::Get()->GetLArGridUVRef();
          if (fPhotonMomentum < 8.3e-6) Actual_Grid_Reflectance = DSParameters::Get()->GetLArGridVisRef();
          if (G4UniformRand() < Actual_Grid_Reflectance) {
...
void G4OpBoundaryProcess::BoundaryProcessVerbose() const
{
  ...
  else if (fStatus == LambertianTransmission)
    G4cout << " *** LambertianTransmission *** " ;
