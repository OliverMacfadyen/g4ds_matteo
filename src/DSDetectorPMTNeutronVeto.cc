...
DSDetectorPMTNeutronVeto::DSDetectorPMTNeutronVeto(G4VPhysicalVolume* myMotherVolume) {
  ...
  // fLogicPMTbulb     = new
  // G4LogicalVolume(fSolidPMTbulb,DSMaterial::Get()->GetStainlessSteel(),"PMTbulb_Logic");
  ...
  //  G4double RadiusPMTbulb =
  //  (RadiusPhotocathode+2*PMT_height+0.5*PHOTOCATHODE_h)*mm; G4double
  //  RadiusPMTbulb = (RadiusPhotocathode + PMT_height - PHOTOCATHODE_h/3. +
  //  PC_slide.mag());
  ...
  vector<G4double> phis = DSParameters::Get()->GetVPMTTheta();  // Inconsistent angle name
  ...
  for (int nPMT = 0; nPMT < (int)DSParameters::Get()->GetNVPMTs(); nPMT++) {
    ...
    // Back of the cathode
    fPhysicPMTbulb = new G4PVPlacement(RotationPMTbulb, RelocationPMTbottom, namebulb, fLogicPMTbulb, fMotherVolume, true, volumeNumber_bulb + nPMT, DSStorage::Get()->GetCheckOverlap());

    DSLog(development) << fPhysicPMTbulb->GetName() << " = " << fPhysicPMTbulb->GetCopyNo() << endlog;

    // MuMetal
    fPhysicPMTbase = new G4PVPlacement(RotationPMTbase, RelocationPMTbase, namebase, fLogicPMTbase, fMotherVolume, true, volumeNumber_base + nPMT, DSStorage::Get()->GetCheckOverlap());

    DSLog(development) << fPhysicPMTbase->GetName() << " = " << fPhysicPMTbase->GetCopyNo() << endlog;

    // Cathode
    fPhysicPhotocathode = new G4PVPlacement(RotationPC, PC_slide, namephotocathode, fLogicPhotocathode, fPhysicPMTbulb, true, volumeNumber_photocathode + nPMT, DSStorage::Get()->GetCheckOverlap());
...
void DSDetectorPMTNeutronVeto::DefineSurfaces() {
  ...
  fOpVPhotocathodeSurface->SetMaterialPropertiesTable(DSMaterial::Get()->GetVPhotocathodeMPT());
  ...
  fOpPMTBackSurface->SetMaterialPropertiesTable(DSMaterial::Get()->GetPMTBackMPT());
  ...
  fPhotocathodeSurface = new G4LogicalBorderSurface("PhotocathodeSurface", fMotherVolume, fPhysicPhotocathode, fOpVPhotocathodeSurface);
  fPMTbaseSurface = new G4LogicalBorderSurface("PMTbaseSurface", fPhysicPMTbase, fMotherVolume, fOpLumirrorSurface);
  fPMTbulbSurface = new G4LogicalBorderSurface("PMTbulbSurface", fPhysicPMTbulb, fMotherVolume, fOpPMTBackSurface);
}

/*
 * $Log: DSDetectorPMTNeutronVeto.cc,v $
 * Revision 1.4  2016/02/26 10:52:02  pagnes
 * un-initialized variable fix
 *
