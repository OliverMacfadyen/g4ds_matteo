...
DSDetectorWaterTank::DSDetectorWaterTank(G4VPhysicalVolume* myMotherVolume) {
  ...
  // Volumes for constructing the WT and LSV sphere
  double myWTankR = 5 * m;
  double myWTankHeight = 4613 * mm;
  double myWaterR = 4995 * mm;
  double myWaterHeight = 4608 * mm;

  if (DSStorage::Get()->Get20KGeometry()) {
    myWTankR = DSStorage::Get()->GetWTankR();
    myWTankHeight = DSStorage::Get()->GetWTankHeight() / 2.;
    myWaterR = myWTankR;
    myWaterHeight = myWTankHeight;
    myWTankR += 1 * cm;
    myWTankHeight += 2 * cm;
  }
  ...
  fSolidSteelTank = new G4Tubs("SteelTank_Solid", 0, myWTankR, myWTankHeight, 0, twopi * rad);
  fLogicSteelTank = new G4LogicalVolume(fSolidSteelTank, DSMaterial::Get()->GetStainlessSteel(), "SteelTank_Logic");
  fPhysicSteelTank = new G4PVPlacement(0, G4ThreeVector(0, 0, 0), "SteelTank", fLogicSteelTank, fMotherVolume, false, 0, DSStorage::Get()->GetCheckOverlap());
  // fLogicSteelTank->SetVisAttributes(G4VisAttributes::GetInvisible());
  // Water volume

  fSolidWaterVolume = new G4Tubs("WaterVolume_Solid", 0, myWaterR, myWaterHeight, 0, twopi * rad);

  if (DSStorage::Get()->GetCTFfill() == 2) {  // BoronScintillator
    fLogicWaterVolume = new G4LogicalVolume(fSolidWaterVolume, DSMaterial::Get()->GetBoronScintillator(), "WaterVolume_Logic");
  } else if (DSStorage::Get()->GetCTFfill() == 1) {  // Water loaded with Gd
    fLogicWaterVolume = new G4LogicalVolume(fSolidWaterVolume, DSMaterial::Get()->GetGdWater(), "WaterVolume_Logic");
  } else if (DSStorage::Get()->GetCTFfill() == 3) {  // Air
    fLogicWaterVolume = new G4LogicalVolume(fSolidWaterVolume, DSMaterial::Get()->GetAir(), "WaterVolume_Logic");
  } else {  // Water
    fLogicWaterVolume = new G4LogicalVolume(fSolidWaterVolume, DSMaterial::Get()->GetWater(), "WaterVolume_Logic");
  }
  fPhysicWaterVolume = new G4PVPlacement(0, G4ThreeVector(0, 0, 0), "WaterVolume", fLogicWaterVolume, fPhysicSteelTank, false, 0, DSStorage::Get()->GetCheckOverlap());

  fLogicWaterVolume->SetVisAttributes(G4VisAttributes::GetInvisible());

  if (DSStorage::Get()->Get20KGeometry()) return;
  ...
  for (int n = 0; n < 4; n++) {
    ...
    fPhysicNVleg = new G4PVPlacement(0, RelocationLeg, nameleg, fLogicNVleg, fPhysicWaterVolume, true, n, DSStorage::Get()->GetCheckOverlap());

    fPhysicNVlegUp = new G4PVPlacement(RotationLegUp, RelocationLegUp, namelegUp, fLogicNVlegUp, fPhysicWaterVolume, true, n, DSStorage::Get()->GetCheckOverlap());

    fPhysicNVbarB = new G4PVPlacement(RotationBar, RelocationBarB, namebarB, fLogicNVbarB, fPhysicWaterVolume, true, n, DSStorage::Get()->GetCheckOverlap());

    fPhysicNVbarH = new G4PVPlacement(RotationBar, RelocationBarH, namebarH, fLogicNVbarH, fPhysicWaterVolume, true, n, DSStorage::Get()->GetCheckOverlap());

    fPhysicNVbarDR = new G4PVPlacement(RotationBar, RelocationBarDRU, namebarDRU, fLogicNVbarDR, fPhysicWaterVolume, true, n, DSStorage::Get()->GetCheckOverlap());

    fPhysicNVbarDR = new G4PVPlacement(RotationBar, RelocationBarDRB, namebarDRB, fLogicNVbarDR, fPhysicWaterVolume, true, n, DSStorage::Get()->GetCheckOverlap());

    fPhysicNVbarDL = new G4PVPlacement(RotationBarL, RelocationBarDLU, namebarDLU, fLogicNVbarDL, fPhysicWaterVolume, true, n, DSStorage::Get()->GetCheckOverlap());

    fPhysicNVbarDL = new G4PVPlacement(RotationBarL, RelocationBarDLB, namebarDLB, fLogicNVbarDL, fPhysicWaterVolume, true, n, DSStorage::Get()->GetCheckOverlap());

    fPhysicNVdisc = new G4PVPlacement(0, RelocationDisc, namedisc, fLogicNVdisc, fPhysicWaterVolume, true, n, DSStorage::Get()->GetCheckOverlap());

    fPhysicNVwing = new G4PVPlacement(RotationWing1, RelocationWing1, namewing1, fLogicNVwing, fPhysicWaterVolume, true, n, DSStorage::Get()->GetCheckOverlap());

    fPhysicNVwing = new G4PVPlacement(RotationWing2, RelocationWing2, namewing2, fLogicNVwing, fPhysicWaterVolume, true, n, DSStorage::Get()->GetCheckOverlap());

    fPhysicNVwing = new G4PVPlacement(RotationWing3, RelocationWing3, namewing3, fLogicNVwing, fPhysicWaterVolume, true, n, DSStorage::Get()->GetCheckOverlap());

    fPhysicNVwing = new G4PVPlacement(RotationWing4, RelocationWing4, namewing4, fLogicNVwing, fPhysicWaterVolume, true, n, DSStorage::Get()->GetCheckOverlap());
  }
}
 ...
 * Revision 1.4  2015/11/18 10:42:41  pagnes
 * overlap corrected (for DS50 with vetoes) and generator bug fixed (DS20k)
 *
 * Revision 1.3  2015/10/31 11:27:00  pagnes
 * added commands to customize WT, DS20k-cryostats and DS20k-LSV geometries
 *
 * Revision 1.2  2015/10/12 16:09:36  pagnes
 * DS20k TPC moved inside CTF
 *
