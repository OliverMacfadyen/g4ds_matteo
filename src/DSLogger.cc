...
ostream& DSLogger::msg(DSLogger::Severity severity, const char* facility) {
  _doPrint = true;
  if (!_myOstream) _myOstream = new ostringstream;
  else
    ((ostringstream*)_myOstream)->str("");
 ...
 * Modified script in order to setup correct environment variables on LNGS
 * cluster Added include cstdlib to handle ::abort function in DSLogger.cc
 *
