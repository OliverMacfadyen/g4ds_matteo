...
DSRunActionMessenger::DSRunActionMessenger(DSRunAction* runAct) : runAction(runAct) {
  ...
  fWriteFilterCmd = new G4UIcmdWithABool("/run/writefilter", this);
...
void DSRunActionMessenger::SetNewValue(G4UIcommand* command, G4String newValue) {
  ...
  } else if (command == fWriteFilterCmd) {
    DSLog(routine) << "Write Filter: " << newValue << endlog;
    // DSOutputVertex::Get()->SetWriteFilter(fWriteFilterCmd->ConvertToBool(newValue));
  }
}
/*
