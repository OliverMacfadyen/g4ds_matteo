...
DSDetectorCalibrationDevice::DSDetectorCalibrationDevice(G4VPhysicalVolume* myMotherVolume) {
  ...
  // a cylindrical hollow tube filled with vacuum (in MC) or N2 (in the
  // detector)
  ...
  // G4double GapSourceCollimator = 0.5 *  cm;  //Source outside the collimator,
  // to avoid that energy is deposited in the collimator (big effect for
  // electrons).
  ...
  */
  ...
  colRot->rotateY(pi / 2 * rad + colRotationAngle);  // rotateY is a rotation around axis Y, the pi/2*rad
                                                     // are from the rotation out of the z-axis
  ...
  // this is a sanity check to be compared with the position in the macro. (It
  // is not used further)
  ...
  // G4ThreeVector colCenter =
  // G4ThreeVector(source_x-(colHalfLength+colEndCapHalfLength*2+GapSourceCollimator)*cos(colRotationAngle*degree),
  // source_y,
  // source_z-(colHalfLength+colEndCapHalfLength*2+GapSourceCollimator)*sin(colRotationAngle*degree));
  ...
  fPhysicCollimator = new G4PVPlacement(colRot, colCenter, "CollimatorVolume", fLogicCollimator, fMotherVolume, false, volumeNumber++, DSStorage::Get()->GetCheckOverlap());
  ...
  // rear end cap
  fPhysicColEndBack = new G4PVPlacement(colRot, colEndBackCenter, "CollimatorBackEndCapVolume", fLogicColEnd, fMotherVolume, false, volumeNumber++, DSStorage::Get()->GetCheckOverlap());

  DSLog(routine) << fPhysicColEndBack->GetName() << " = " << fPhysicColEndBack->GetCopyNo() << endlog;

  // front end cap
  fPhysicColEndFront = new G4PVPlacement(colRot, colEndFrontCenter, "CollimatorFrontEndCapVolume", fLogicColEnd, fMotherVolume, false, volumeNumber++, DSStorage::Get()->GetCheckOverlap());
  ...
  // fLogicColFill = new G4LogicalVolume(fSolidColFill,
  // DSMaterial::Get()->GetVacuum(), "CollimatorFill_Logic");
  fLogicColFill = new G4LogicalVolume(fSolidColFill, DSMaterial::Get()->GetGaseousNitrogen(), "CollimatorFill_Logic");
  fPhysicColFill = new G4PVPlacement(colRot, colCenter, "CollimatorFillVolume", fLogicColFill, fMotherVolume, false, volumeNumber++, DSStorage::Get()->GetCheckOverlap());
