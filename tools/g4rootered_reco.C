...
#include "reco/fityMDF.cxx"
#include "reco/fitxMDF.cxx"
#include "reco/pca.C"
...
*/
  ...
  dstree->Branch("xreco",              &xreco,     "xreco/F");	   
  dstree->Branch("yreco",              &yreco,     "yreco/F");	   
  ...
  // initialize MDF objects for reconstruction
  fitx *mfitX = new fitx;
  fity *mfitY = new fity;
  
  // initialize arrays for PCAMDF data
  double channels[19];
  double data[19];
  ...
  for(int _i=0; _i<nevents; _i++) {  
    ...
    for(int i = 0; i < theEvent.NPE; ++i) {
      ...
      if(pe_time[i] < 10000){
        ...
        NPES2++; // otherwise, increment NPES2
      }
    ...
    // get S2 fractions, accounting for 1PE baseline across all PMTs
    for(int i = 0; i < 19; ++i){
    ...
    // recover radius-normalized XY position
    X2P(channels,data);
    xreco = mfitX->MDF(data) * 17.78;
    yreco = mfitY->MDF(data) * 17.78;
