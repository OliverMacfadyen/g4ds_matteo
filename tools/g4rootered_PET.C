...
*/
  ...
  dstree->Branch("nph1",        nph1,                    "nph1[2]/I");
  dstree->Branch("nph2",        nph2,                    "nph2[2]/I");
  dstree->Branch("dene",        dene,                    "dene[2]/F");
  ...
  for(int _i=0; _i<nevents; _i++) {  
    ...
    for(int i=0;i<theEvent.NDeposits;++i) {
      ...
      if(dep_mat[i] == 8) {
        if(dep_z[i] > 0) dene[0] += dep_ene[i];       
        else             dene[1] += dep_ene[i]; 
      }
    }    
    // Fill user variables
    ...
    for(int i=0;i<2;++i) { nph1[i] = 0 ; nph2[i] = 0;}
    ...
    for(int i=0;i<2;++i) { 
      if(dene[i] > 0) doWrite = true ;
      //cout << nph1[i] << " " << nph2[i] << endl ;
      if(nph1[i] > 0) doWrite = true ; 
      if(nph2[i] > 0) doWrite = true ; 
    }
        
    //doWrite = true ;
    // Fill the event
    if(doWrite) dstree->Fill();
...
/*
 * $Log: g4rootered_PET.C,v $
 * Revision 1.1  2016/02/05 16:51:16  pagnes
