...
const int MAXDAUGHTER = 7000;
// const int MAXDEPOSIT  = 10000;
const int MAXDEPOSIT  = 100000;
const int MAXNPH      = 242000;
const int MAXNPE      = 200000;
...
bool _readEvent (ifstream *file) {
	...
	if(theEvent.NPH        > MAXNPH)      { cout << "Fatal: NPH > MAXNPH " << MAXNPH << endl ; exit(0) ;}
...
int main (int argc, char *argv[]) {
	...
	float LY        = 500.;
        ...
        double dart_r = 5.0;        // DART radius in cm
        double fvcut = dart_r-0.3;  // remove space between Cu vessel and reflector, in cm
	...
	double ph_time[MAXNPH]; 
        ...
        ///Reescribir como array
        float ph_ch1, ph_ch2, ph_ch3, ph_ch4, ph_ch5, ph_ch6, ph_ch7, ph_ch8;

    	int LArIndex;
	float s1ene,s2ene, veto_visene, mu_visene, ene, qene, qnpe, tpcene, fvtpcene, vetoene, muene, missene;
	int ndepTPC;
	float ene_non,eneTPC,f90TPC,nphvuv,nphvis,npevis; 
	...
	dstree->Branch("ene_nonscint",   &ene_non,                  "ene_non/F");	
	...
	dstree->Branch("tpcene",         &tpcene,  "tpcene/F"); 
	dstree->Branch("missene",         &missene,  "missene/F");         
	dstree->Branch("fvtpcene",       &fvtpcene,"fvtpcene/F");     
	dstree->Branch("vetoene",        &vetoene, "vetoene/F");     
	...
	dstree->Branch("nphvuv",            &nphvuv,	      "nphvuv/F");        
	dstree->Branch("nphvis",            &nphvis,	      "nphvis/F");   
	dstree->Branch("npevis",            &npevis,	      "npevis/F");   
	dstree->Branch("ph_ch1",            &ph_ch1,	      "ph_ch1/F");   
	dstree->Branch("ph_ch2",            &ph_ch2,	      "ph_ch2/F");   
	dstree->Branch("ph_ch3",            &ph_ch3,	      "ph_ch3/F");   
	dstree->Branch("ph_ch4",            &ph_ch4,	      "ph_ch4/F");   
	dstree->Branch("ph_ch5",            &ph_ch5,	      "ph_ch5/F");   
	dstree->Branch("ph_ch6",            &ph_ch6,	      "ph_ch6/F");   
	dstree->Branch("ph_ch7",            &ph_ch7,	      "ph_ch7/F");   
	dstree->Branch("ph_ch8",            &ph_ch8,	      "ph_ch8/F");   
	...
	for(int _i=0; _i<nevents; _i++) {  
		...
		if (!(counter % 10000) && counter>0) 
			std::cout << counter <<" processed events " << " (event id = " <<  theEvent.EventID << ")"<< std::endl;
		counter++ ;
		...
		for(int i=0;i<theEvent.NDeposits;++i) {
			...
			if(dep_mat[i] == 9) vetoene += dep_ene[i];

			// if(dep_mat[i] == 8) tpcene += dep_ene[i];
                        if (dep_mat[i] == 7 || dep_mat[i]==10) missene += dep_ene[i];
			if(dep_mat[i] == 8) {
                           if(firstTPC) {
                             firstTPC = false ;
                             timeTPC  = dep_time[i] ;
                           ...
                           else depNucl += (dep_ene[i],1.)*0.25;

                           // total energy in DART
			   tpcene += dep_ene[i];
			   if (dep_r[i] <= fvcut) fvtpcene += dep_ene[i];
                        }
     ...
     // TPC
		...
		for(int i=0;i<theEvent.MuNPE;++i) {
			...
			mu_pe_time[i] = theMuPhotoElectrons[i].Time;    
                ...
                nphvis=0;
                npevis=0;
                ph_ch1 = ph_ch2 = ph_ch3 = ph_ch8 = ph_ch4 = ph_ch5 = ph_ch6 = ph_ch7 = 0;
		...
		for(int i=0;i<theEvent.NPH;++i) {
			...
			ph_time[i]   = thePhotons[i].Time;    
                        if (thePhotons[i].Wavelength < 200) nphvuv = nphvuv+1;
                        if (thePhotons[i].Wavelength > 200) nphvis = nphvis+1;
                        if (thePhotons[i].VolumeID == 56 && thePhotons[i].Wavelength > 200 ) npevis = npevis+1;
                        
                        if (thePhotons[i].Position[2] > 0){
                           if (thePhotons[i].VolumeID == 56 && thePhotons[i].Wavelength > 200 && thePhotons[i].Position[0] < 0 && thePhotons[i].Position[1] > 0 ) ph_ch1++;
                           if (thePhotons[i].VolumeID == 56 && thePhotons[i].Wavelength > 200 && thePhotons[i].Position[0] > 0 && thePhotons[i].Position[1] > 0 ) ph_ch2++;
                           if (thePhotons[i].VolumeID == 56 && thePhotons[i].Wavelength > 200 && thePhotons[i].Position[0] < 0 && thePhotons[i].Position[1] < 0 ) ph_ch3++;
                           if (thePhotons[i].VolumeID == 56 && thePhotons[i].Wavelength > 200 && thePhotons[i].Position[0] > 0 && thePhotons[i].Position[1] < 0 ) ph_ch4++;
                        }

                        if (thePhotons[i].Position[2] < 0){
                           if (thePhotons[i].VolumeID == 56 && thePhotons[i].Wavelength > 200 && thePhotons[i].Position[0] < 0 && thePhotons[i].Position[1] > 0 ) ph_ch5++;
                           if (thePhotons[i].VolumeID == 56 && thePhotons[i].Wavelength > 200 && thePhotons[i].Position[0] > 0 && thePhotons[i].Position[1] > 0 ) ph_ch6++;
                           if (thePhotons[i].VolumeID == 56 && thePhotons[i].Wavelength > 200 && thePhotons[i].Position[0] < 0 && thePhotons[i].Position[1] < 0 ) ph_ch7++;
                           if (thePhotons[i].VolumeID == 56 && thePhotons[i].Wavelength > 200 && thePhotons[i].Position[0] > 0 && thePhotons[i].Position[1] < 0 ) ph_ch8++;
                        }
...
0;
	 ...
	 * New version of g4ds
	 * Revision 1.15 2018/01/19 13:36 esanchez 
         * New variable missene
