...
int main (int argc, char *argv[]) {
  ...
  float baf511_ene, baf1270_ene, bege_ene;
  ...
  dstree->Branch("baf511_ene", &baf511_ene, "baf511_ene/F");
  dstree->Branch("baf1270_ene", &baf1270_ene, "baf1270_ene/F");
  dstree->Branch("bege_ene", &bege_ene, "bege_ene/F");
  ...
  for(int _i=0; _i<nevents; _i++) {
	...
	if(dep_pdg[i]< 30) qdepVeto += dep_ene[i];
      ...
      // ARIS-ER gamma detectors
      if(dep_mat[i] == 85)  baf511_ene += dep_ene[i];
      if(dep_mat[i] == 86)  baf1270_ene += dep_ene[i];
      if(dep_mat[i] == 89)  bege_ene += dep_ene[i];
